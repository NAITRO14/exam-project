...
void dataManager::create_base()
{
    ...
    (*users)[17].addInterests({ "Шахматы", "Коллекционирование", "Астрономия" });
    (*users)[18].addInterests({ "Книги", "Игры", "Футбол" });
    (*users)[19].addInterests({ "Программирование", "Спорт", "Фильмы" });
    (*users)[20].addInterests({ "Йога", "Виноделие", "Путешествия" });
}

void dataManager::check_base()
{
    QDir dir("appData");
    if (!dir.exists()) {
        dir.mkpath(".");
    ...
    QString usersPath = dir.absoluteFilePath("users.txt");
    QString interestsPath = dir.absoluteFilePath("users_interests.txt");

    QFile checkFile(usersPath);
    if (!checkFile.exists()) {
        create_base();

        // Запись users.txt
        QFile file1(usersPath);
...
void dataManager::updateData_inFile()
{
    // Запись users.txt
